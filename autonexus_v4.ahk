...
if WinExist("RotMGExalt")
{
	...
	calculatedHeight := Height * 0.425
	calculatedWidth := Width * 0.82
	calculatedConst := ((Width * 0.9911) - calculatedWidth)/100
	...
	PixelGetColor, colorpot, %potValueCeil%, %calculatedHeight%
    	PixelGetColor, colorNexus, %nexusValueCeil%, %calculatedHeight%
	if ("0xFFFFFF" = colorNexus or "0x545454" = colorNexus) {
		Send %nexusHotkey%
	}
	if (!Cooldown){ 
		if ("0xFFFFFF" = colorpot or "0x545454" = colorpot) {
			Send %potHotkey%
			Cooldown := true
			SetTimer, Cooldown, 350
		}
	}
	Sleep, 15 
}
